...
class Form(StatesGroup):
    ...
    location = State()
    description = State()
    photo = State()           # ← ЭТО БЫЛО ПРОПУЩЕНО — ИЗ-ЗА ЭТОГО КРАШ
    done_comment = State()
...
async def get_photo(message: Message, state: FSMContext):
    ...
    user_requests[user_id].append({"date": datetime.now().strftime("%d.%m %H:%M"), "park": park, "location": location, "description": desc})
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Проблема решена", callback_data=f"done_{user_id}")]])
...
async def problem_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Напишите комментарий и/или прикрепите фото результата:")
...
async def get_done_comment(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Уведомление отправлено.", reply_markup=main_menu)
    await state.clear()
